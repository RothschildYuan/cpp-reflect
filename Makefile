PROGRAM = CppReflect             # 最后的可执行文件

# 通配当前目录下.cpp 和 .c文件
CFILES := $(shell find test -mindepth 1 -maxdepth 4 -name "*.c")
CXXFILES := $(shell find test -mindepth 1 -maxdepth 4 -name "*.cpp")
CXXFILES += $(shell find reflect -mindepth 1 -maxdepth 4 -name "*.cpp")
# 输入文件
